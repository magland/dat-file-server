...
function DatWidget() {
	...
	this.datKey=function() {return m_dat_key;};
	this.setDatKey=function(key) {m_dat_key=key; m_current_directory=''; refresh();};
	...
	function refresh() {
		...
		//var str=`dat://${shorten_key(m_dat_key,10)}/${m_current_directory}`;
		...
		$.getJSON(`/../api/readdir/${m_dat_key}/${m_current_directory}`,function(resp) {
			if (resp.error) {
				throw resp.error;
			}
			var dirs=resp.dirs;
	...
	function create_path_element() {
		...
		path_element.append(`<a href=# data-path="">dat://${shorten_key(m_dat_key,10)}</a>`);
	...
	function update_file_row(row) {
		...
		var url=`/download/${m_dat_key}/${filepath}`;
		link.attr('href',url);
		row.find('#name').empty();
